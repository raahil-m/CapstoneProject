  ...
  class NPCController extends entity.Component {
    ...
    _LoadModels() {
      ...
      loader.load(this._params.resourceName, (glb) => {
        ...
        this._target.scale.setScalar(0.325);
        ...
        loader.setPath('./resources/monsters/FBX/');
        loader.load('MutantGooseIdle.fbx', (a) => { _OnLoad('idle', a); });
        loader.load('GooseFlyLoop.fbx', (a) => { _OnLoad('walk', a); });
        loader.load('GooseNeckBite.fbx', (a) => { _OnLoad('attack', a); });
        loader.load('GooseStandingTorchJumpRunning.fbx', (a) => { _OnLoad('death', a); });
      });
    }
    ...
    _FindIntersections(pos) {
      const _IsAlive = (c) => {
        const h = c.entity.GetComponent('GooseComponent');
        if (!h) {
          return true;
        }
        return h._health > 0;
      ...
      for (let i = 0; i < nearby.length; ++i) {
        ...
        // Hardcode the following if statement
        if (d <= 4) {
